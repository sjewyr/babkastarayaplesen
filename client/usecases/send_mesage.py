...
def send_message_usecase(client_id: int, message: str):
    ...
    msg = IncomingMessage(subject=os.getenv("CLIENT_NAME"), message=message, signature = sign, timestamp=int(time.time()), public_keys= public_keys, certificate=my_ca, root_ca=root_ca, ca_ca=ica_ca)
    rs = requests.post(url=f"http://client{str(client_id)}:8000/message/get_message", json = msg.model_dump()) # подумать
